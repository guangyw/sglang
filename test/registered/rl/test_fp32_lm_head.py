...
class TestLMHeadFP32(unittest.TestCase):
    ...
    def _make_logprocessor(self, vocab_size, enable_fp32):
        # LogitsProcessor reads enable_fp32_lm_head from the resolved config bag
        # (get_exec().features), so set it at construction and publish — a bare
        # post-publish server_args write would not reach the projected bag.
        set_global_server_args_for_scheduler(
            ServerArgs(model_path="dummy", enable_fp32_lm_head=enable_fp32)
        )
